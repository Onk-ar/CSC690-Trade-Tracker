...
//  trade tracker
//
//  Created by Onkar Singh on 5/13/21.
//
...
struct CalcService {
    ...
    func calculate(asset: Asset,
                   ...
                   initialDateOfInvestmentIndex: Int) -> CalcResult {
    ...
    private func getNumberOfShares(asset: Asset,
        ...
        asset.timeSeriesMonthlyAdjusted.getMonthInfos().prefix(initialDateOfInvestmentIndex).forEach { (monthInfo) in
            let invShares = monthlyDollarCostAveragingAmount / monthInfo.adjustedOpen
            totalShares += invShares
        }
        return totalShares
    }
